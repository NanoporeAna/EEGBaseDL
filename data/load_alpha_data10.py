...
with open("H:/SpaceWork/EEG_Work/Alpha_path10") as file_object:
...
def alpha_data(num):
    ...
    for i in range(0, int(shape/10000)):#存储第一个人的数据，将其作为测试集
        batch = load_matrix[i * 10000:(i + 1) * 10000]  # 取500*9的数据矩阵
...
def load_alpha_data_pool3(path,count):
    ...
    for i in range(0, int(shape / 8) - 1):
        tem = (alpha_data[i * 8] + alpha_data[i * 8 + 1] + alpha_data[i * 8 + 2] + alpha_data[i * 8 + 3] + alpha_data[
            i * 8 + 4]+alpha_data[i * 8+5] + alpha_data[i * 8 + 6] + alpha_data[i * 8 + 7] + alpha_data[i * 8 + 8] ) / 9
        temp.append(tem)
    temp.append(alpha_data[shape - 1])
    data = np.array(temp)
    return data
def load_alpha_data_pool4(path,count):
    ...
    for j in range(0, flg):
        for i in range(0, 312):
            tem = (alpha_data[i * 16] + alpha_data[i * 16 + 1] + alpha_data[i * 16 + 2] + alpha_data[i * 16 + 3] +
                   alpha_data[i * 16 + 4] + alpha_data[i * 16 + 5] + alpha_data[i * 16 + 6] + alpha_data[i * 16 + 7] +
                   alpha_data[i * 16 + 8] + alpha_data[i * 16 + 9] + alpha_data[i * 16 + 10] + alpha_data[i * 16 + 11] +
                   alpha_data[i * 16 + 12] + alpha_data[i * 16 + 13] + alpha_data[i * 16 + 14] + alpha_data[
                       i * 16 + 15]) / 16
            temp.append(tem)
        flag = (alpha_data[j * 5000 + 4992] + alpha_data[j * 5000 + 4993] + alpha_data[j * 5000 + 4994] + alpha_data[
            j * 5000 + 4995]
                + alpha_data[j * 5000 + 4996] + alpha_data[j * 5000 + 4997] + alpha_data[j * 5000 + 4998] + alpha_data[
                    j * 5000 + 4999]) / 8
        temp.append(flag)
    data = np.array(temp)
    return data
