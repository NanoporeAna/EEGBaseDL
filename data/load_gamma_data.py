...
with open("H:/SpaceWork/EEG_Work/Gamma_paths") as file_object:
...
def load_gamma_data_pool3(path,count):
    ...
    for j in range(0, flg):
        for i in range(0,62):
            tem = (gamma_data[j*500+i*8]+gamma_data[j*500+i*8+1]+gamma_data[j*500+i*8+2]+gamma_data[j*500+i*8+3]+gamma_data[j*500+i*8+4]+
            gamma_data[j*500+i * 8+5] + gamma_data[j*500+i * 8 + 6] + gamma_data[j*500+i * 8 + 7]  )/8
            temp.append(tem)
        flag = (gamma_data[j*500+496]+gamma_data[j*500+497]+gamma_data[j*500+498]+gamma_data[j*500+499])/4
        temp.append(flag)
    data =np.array(temp)
    return data
...
def load_gamma_data_pool4(path,count):
    ...
    for j in range(0, flg):
        for i in range(0, 31):
            # tem = (gamma_data[j * 500 + i * 16] + gamma_data[j * 500 + i * 16 + 1] + gamma_data[j * 500 + i * 16 + 2] +
            #        gamma_data[j * 500 + i * 16 + 3] + gamma_data[j * 500 + i * 16 + 4] +
            #        gamma_data[j * 500 + i * 16 + 5] + gamma_data[j * 500 + i * 16 + 6] + gamma_data[
            #            j * 500 + i * 16 + 7] + gamma_data[j * 500 + i * 16 + 8] + gamma_data[j * 500 + i * 16 + 9] +
            #        gamma_data[j * 500 + i * 16 + 10] + gamma_data[j * 500 + i * 16 + 11] + gamma_data[
            #            j * 500 + i * 16 + 12] +
            #        gamma_data[j * 500 + i * 16 + 13] + gamma_data[j * 500 + i * 16 + 14] + gamma_data[
            #            j * 500 + i * 16 + 15]) / 16
            # temp.append(tem)
            tem = (gamma_data[j * 500 + i * 16] + gamma_data[j * 500 + i * 16 + 1] + gamma_data[j * 500 + i * 16 + 2] +
                   gamma_data[j * 500 + i * 16 + 3] + gamma_data[j * 500 + i * 16 + 4] +
                   ...
                   gamma_data[j * 500 + i * 16 + 10] + gamma_data[j * 500 + i * 16 + 11] + gamma_data[
                       j * 500 + i * 16 + 12] +
                   gamma_data[j * 500 + i * 16 + 13] + gamma_data[j * 500 + i * 16 + 14]+ gamma_data[j * 500 + i * 16 + 15]) / 16
            temp.append(tem)
        flag = (gamma_data[j * 500 + 496] + gamma_data[j * 500 + 497] + gamma_data[j * 500 + 498] + gamma_data[
            j * 500 + 499]) / 4
        temp.append(flag)
