...
from data.load_gamma_data10 import load_gamma_data, load_gamma_data_pool1, load_gamma_data_pool2, load_gamma_data_pool3, \
    load_gamma_data_pool4
from data.load_single_data10 import test_batch
...
Model_Save_Path = "H:/SpaceWork/EEG_Work/Model/CNN10_BN_v1"
...
def get_corr_fliter(num,data,channle,fliter,pool):
    ...
    for i in range(channle):
        ...
        for j in range(9):
            if pool ==0:
                ga = load_gamma_data(num,j)#获取要进行相关性分析的对象数据
            if pool>0 and pool <=2:
                ga = load_gamma_data_pool1(num,j)
            if pool>2 and pool <=4:
                ga = load_gamma_data_pool2(num,j)
            if pool>4 and pool <=6:
                ga = load_gamma_data_pool3(num,j)
            if pool >6:
                ga = load_gamma_data_pool4(num,j)
            cost = []
...
def sum(num):
    ...
    sio.savemat('D:/EEG_WorkSpace10/MRS/data_corr/gamma/'+str(num)+'.mat', {name1[0]:channel[0], name1[1]:channel[1], name1[2]:channel[2], name1[3]:channel[3], name1[4]:channel[4],name1[5]:channel[5],name1[6]:channel[6], name1[7]:channel[7]})
