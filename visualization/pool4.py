...
from data.load_alpha_data import load_alpha_data_pool4
from data.load_beta_data import load_beta_data_pool4
from data.load_delta_data import load_delta_data, load_delta_data_pool1, load_delta_data_pool2, load_delta_data_pool3, \
	load_delta_data_pool4
from data.load_gamma_data import load_gamma_data_pool4
from data.load_single_data import test_batch
from data.load_theta_data import load_theta_data_pool4
...
def get_corr_fliter(num,data,channle,fliter,pool):
    ...
    for i in range(channle):
        ...
        for j in range(9):
            ...
            if pool > 6:
	            ga = load_theta_data_pool4(num,j)
            cost = []
...
def sum(num):
    ...
    sio.savemat('D:/pool4/theta/'+str(num)+'.mat', {name1[7]:channel[7]})
